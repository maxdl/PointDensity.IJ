...
public class PointDensity_ extends PlugInFrame implements OptionsPD, ActionListener {
    ...
    Label commentLabel;
    Label scaleLabel;
    ProfileDataPD profile;
    ImagePlus imp;    

   
    public PointDensity_() {
        super("PointDensity");
        if (instance != null) {
            instance.toFront();
            return;
        ...
        instance = this;
        profile = new ProfileDataPD();
        IJ.register(PointDensity_.class);
    ...
    PolygonRoi getPolygonRoi(ImagePlus imp) {
        ...
        if (roi == null || roi.getType() != Roi.POLYGON) {
            IJ.error("PointDensity", "Polygon selection required.");
            return null;
        } else {
    ...
    PolygonRoi getPointRoi(ImagePlus imp) {
        ...
        if (roi == null || roi.getType() != Roi.POINT) {
            IJ.error("PointDensity", "Point selection required.");
            return null;
        } else {
            return (PolygonRoi) roi;
        }
...
} // end of PointDensity_

class ProfileDataPD implements OptionsPD {
    boolean dirty;
    Overlay overlay;
    ...
    public void deleteSelectedComponent(ImagePlus imp) {
        if (!this.overlay.contains(imp.getRoi())) {
            ...
            YesNoCancelDialog d = new YesNoCancelDialog(imp.getWindow(),
                    "PointDensity", "Delete " + imp.getRoi().getName() + "?");
            if (d.yesPressed()) {
    ...
    public boolean isSameImage(ImagePlus imp) {
        if (!this.dirty || this.imgID == 0) {
            ...
            IJ.error("PointDensity", "All measurements must be performed on the same image.");
            return false;
        }
    }
    ...
    private boolean CheckProfileData(ImagePlus imp) {
        ...
        if (nerr > 0) {
            IJ.error("PointDensity", "Error:\n" + errstr[0]);
            return false;
        }
        if (nwarn > 0) {
            for (i = 0; i < nwarn; i++) {
                YesNoCancelDialog d = new YesNoCancelDialog(imp.getWindow(),
                        "PointDensity", "Warning:\n" + warnstr[i] + "\nContinue anyway?");
                if (!d.yesPressed()) {
                    return false;
                }
            }
